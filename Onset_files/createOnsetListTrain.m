function [averageDiff, maximumDiff, minimumDiff, diff] = createOnsetListTrain(mean_t,min_t,max_t,interval,requiredLength,numTrials,numOnsetlists)
% -------------------------------------
% Created by Rotem Botvinik 4/12/14
% Edited by Rotem Botvinik 11/12/14
% Edited by Rani Gera 07/16
% -------------------------------------

%!!!!!!!!!!! The line I (Rani) used for Training: createOnsetList(3,1,10,1,70,15,4)!!!!!!!!
% createOnsetListTrain(1,0,5,1,45,16,4)
% createOnsetListTrain(MeanJitter,2,7,1,(MeanJitter+2)*(NumberOfFractals-1),NumberOfFractals,NumOfOnsetsLists)
...
for listNum = 1:numOnsetlists
    ...
    while onsets(end) ~= requiredLength
...
%         while averageDiff(listNum) ~= mean_t
        ...
        for i = 2:numTrials
            onsets(i)=onsets(i-1)+diff(listNum,i-1);
    ...
    onsets = onsets + 2;
    % MyEDITING
    save(['train_onset_' num2str(listNum)],'onsets');
